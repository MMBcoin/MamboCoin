...
const string strMessageMagic = "MamboCoin Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees, int nHeight)
{
    ...
    if (pindexBest->nMoneySupply < MAX_MONEY){
        if(pindexBest->nHeight == 0) //Premine 4% 3.4M
        {
        ...
        {
            nSubsidy = 30 * COIN;
        }else if(pindexBest->nHeight <= 405000)
        {
            nSubsidy = 15 * COIN;
        }
    }
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees, int nHeight)
{
    int64_t nSubsidy = 0 * COIN;
    if (pindexBest->nMoneySupply < MAX_MONEY){
        if(pindexBest->nHeight >= 101 && pindexBest->nHeight<60001 )
        {
            nSubsidy = 25 * COIN;
        }
        ...
        {
            nSubsidy = 65 * COIN;
        }else if(pindexBest->nHeight <= 255000)
        {
            nSubsidy = 100 * COIN;
        }else if(pindexBest->nHeight <= 305000)
        {
            nSubsidy = 65 * COIN;
        }else if(pindexBest->nHeight <= 355000)
        {
            nSubsidy = 50 * COIN;
        }else
        {
...
#ifdef ENABLE_WALLET
// MamboCoin: attempt to generate suitable proof-of-stake
...
{
    RenameThread("MamboCoin-loadblk");
...
CScript GetDeveloperScript()
{
  ...
  string strAddress = TestNet ? DEVELOPER_ADDRESS_TESTNET : DEVELOPER_ADDRESS;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue)
{
    ...
    return (blockValue - nDeveloperPayment) * 80 / 100; //80%
