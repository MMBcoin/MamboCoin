...
void Shutdown()
{
    ...
    RenameThread("mambocoin-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: MamboCoin.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: mambocoind.pid)") + "\n";
    ...
    strUsage += "  -port=<port>           " + _("Listen for connections on <port> (default: 21411 or testnet: 21412)") + "\n";
    ...
    strUsage += "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 21411 or testnet: 21413)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeMamboCoinamount=<n> " + _("Keep N MamboCoin anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. MamboCoin is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. MamboCoin is probably already running."), strDataDir));
    ...
    if (fDaemon)
        fprintf(stdout, "MamboCoin server starting\n");
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of MamboCoin") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart MamboCoin to complete") << "\n";
    ...
    nAnonymizeMamboCoinAmount = GetArg("-anonymizeMamboCoinamount", 0);
    if(nAnonymizeMamboCoinAmount > 999999) nAnonymizeMamboCoinAmount = 999999;
    if(nAnonymizeMamboCoinAmount < 2) nAnonymizeMamboCoinAmount = 2;
    ...
    if(fEnableInstantX){
        ...
        if(nInstantXDepth < 0) nAnonymizeMamboCoinAmount = 0;
    } else {
        nInstantXDepth = 0;
    }
