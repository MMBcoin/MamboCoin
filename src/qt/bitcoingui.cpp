    ...
    nWeight(0)
{
    //resize(970, 570);
	resize(1080, 662);
    setWindowTitle(tr("MamboCoin") + " - " + tr("Wallet"));
#ifndef Q_OS_MAC
...
#endif
    setObjectName("MamboCoin");
    setStyleSheet("#MamboCoin { background-color: #f9f9f9);  }"
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setToolTip(tr("Send coins to a MamboCoin address"));
    ...
    masternodeManagerAction = new QAction(QIcon(":/icons/bitcoin"), tr("&MamboNodes"), this);
    masternodeManagerAction->setToolTip(tr("Show MamboCoin Nodes status and configure your nodes."));
...
void BitcoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if(clientModel)
    {
        ...
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("MamboCoin client") + QString(" ") + tr("[testnet]"));
...
void BitcoinGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("MamboCoin client"));
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to MamboCoin network", "", count));
}
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid MamboCoin address or malformed URI parameters."));
    }
