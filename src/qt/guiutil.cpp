...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("MamboCoin"))
        return false;
...
{
    // Convert MamboCoin:// to MamboCoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("MamboCoin://"))
    {
        uri.replace(0, 12, "MamboCoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "MamboCoin.lnk";
}
...
{
    return GetAutostartDir() / "MamboCoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("MamboCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  MamboCoin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    uiOptions = tr("UI options") + ":\n" +
        ...
        "  -disablereportupdate   " + tr("Disable auto update of roi report window (default: 0)") + "\n";


    setWindowTitle(tr("MamboCoin-Qt"));
