...
int nAnonymizeMamboCoinAmount = 500;
...
#else
    const char* pszModule = "MamboCoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\MamboCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\MamboCoin
    // Mac: ~/Library/Application Support/MamboCoin
    // Unix: ~/.MamboCoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MamboCoin";
#else
    fs::path pathRet;
...
void createConf()       //Automatic MamboCoin.conf generation
{
    srand(time(NULL));
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "MamboCoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "mambocoind.pid"));
